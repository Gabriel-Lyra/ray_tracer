...
func castRay(orig, dir r3.Vector, spheres []Sphere, lights []Light, depth int, envmap image.Image) color.RGBA {
	...
	depth += 1
	if depth > 10 || !sceneIntersect(orig, dir, spheres, &point, &N, &material) { // depth para reflexões
...
func main() {
	...
	spheres = append(spheres, Sphere{r3.Vector{-3.0, 0.0, -16.0}, 0, ivory})
	spheres = append(spheres, Sphere{r3.Vector{-4.0, -1.5, -12.0}, 2, glass})
	spheres = append(spheres, Sphere{r3.Vector{1.5, -0.5, -18.0}, 3, redRubber})
	spheres = append(spheres, Sphere{r3.Vector{11.0, 5.0, -18.0}, 6, mirror})
